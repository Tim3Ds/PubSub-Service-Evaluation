...
int main(int argc, char* argv[]) {
    ...
    std::cout << " [*] Receiver " << receiver_id << " waiting for messages on " << channel << std::endl;
        ...
        if (status == REDIS_OK && reply) {
            if (reply->type == REDIS_REPLY_ARRAY && reply->elements >= 3) {
                if (reply->element[0]->type == REDIS_REPLY_STRING && 
                    ...
                    std::cout << " [x] Raw message received (" << message_str.length() << " bytes)" << std::endl;
            ...
            if (c_sub->err == REDIS_ERR_IO && (errno == EAGAIN || errno == EINTR || errno == 0)) {
                // This is a timeout or interrupted. Clear error to allow retry.
                // In some systems, hiredis sets err=REDIS_ERR_IO and errno=0 for timeout.
                c_sub->err = 0;
